...
public class AlertDeliveryService {
  ...
  public AlertDeliveryService(AlertRepository logs, SmtpEmailClient mail, RestClient.Builder builder,
    ...
    this.logs = logs; this.mail = mail; this.http = builder.build(); this.json = json; this.sender = sender;
  ...
  private void sendWhatsApp(AlertLog alert) throws Exception {
    ...
    http.post().uri("https://graph.facebook.com/" + graphVersion + "/" + whatsappPhoneId + "/messages")
        .headers(h -> h.setBearerAuth(whatsappToken)).header("Content-Type", "application/json").body(body).retrieve().toBodilessEntity();
  }
}
